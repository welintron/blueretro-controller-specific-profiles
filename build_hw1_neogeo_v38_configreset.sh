...
CONTAINER="blueretro-build-hw1-neogeo-v38-$$"
OUT="$PROJECT_DIR/artifacts/neogeo/BlueRetro_hw1_neogeo_v38_configreset.bin"
...
cp configs/hw1/parallel_1p sdkconfig
find /work -path /work/build -prune -o -type f -exec touch {} +
. "$IDF_PATH/export.sh"
'

echo "[6/7] Building HW1 / Neo Geo (Parallel 1P)..."
docker exec "$CONTAINER" /bin/bash -lc '
set -euo pipefail
cd /work
. "$IDF_PATH/export.sh"
export BR_HW=_hw1
export BR_SYS=_parallel_1p
idf.py build
'

echo "[7/7] Copying firmware artifacts..."
mkdir -p "$PROJECT_DIR/artifacts/neogeo"
docker cp "$CONTAINER:/work/build/BlueRetro_hw1_parallel_1p.bin" "$OUT"
docker cp "$CONTAINER:/work/build/bootloader/bootloader.bin" "$PROJECT_DIR/artifacts/neogeo/bootloader.bin"
docker cp "$CONTAINER:/work/build/partition_table/partition-table.bin" "$PROJECT_DIR/artifacts/neogeo/partition-table.bin"
docker cp "$CONTAINER:/work/build/ota_data_initial.bin" "$PROJECT_DIR/artifacts/neogeo/ota_data_initial.bin"
