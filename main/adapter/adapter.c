...
void adapter_bridge(struct bt_data *bt_data) {
    ...
    if (bt_data->base.pids->type != BT_NONE) {
        ...
        if (wired_adapter.system_id != WIRED_AUTO) {
            ...
            /*
             * If this controller has a persistent MAC-keyed profile, load
             * it into the runtime slot before mapping. If no profile exists,
             * retain the original slot-based configuration.
             */
            /*
             * Create the MAC-keyed profile on first successful HID
             * connection. This must happen while the controller is
             * connected because Web Config is unavailable then.
             */
            config_bt_profile_ensure(
                bt_data->base.pids->id,
                bt_data->base.pids->out_idx);

            {
                int32_t apply_result = config_bt_profile_apply(
                    bt_data->base.pids->id,
                    bt_data->base.pids->out_idx);
                printf("# BT_PROFILE_APPLY dev=%ld out=%ld result=%ld layer=%s\n",
                       (long)bt_data->base.pids->id,
                       (long)bt_data->base.pids->out_idx,
                       (long)apply_result,
                       apply_result == -2 ? "OVERRIDE" :
                       (config_get_src() == DEFAULT_CFG ? "GLOBAL" : "GAMEID"));
            }
