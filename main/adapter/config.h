...
/* Maximum number of persistent Bluetooth-controller mapping profiles. */
#define BT_PROFILE_MAX 7
...
/*
 * Persistent controller-specific mapping profiles.
 *
 * A profile is keyed by the controller Bluetooth address.  The profile
 * mapping is copied into the runtime input slot when that controller is
 * active, so the existing Web Config protocol remains compatible.
 */
...
uint32_t config_bt_profile_v30_event_diag(uint8_t *data, uint32_t max_len);
uint32_t config_bt_profile_v30_snapshot_diag(uint8_t *data, uint32_t max_len);
...
uint32_t config_bt_profile_v32_ring_event(uint8_t logical_index, uint8_t *data, uint32_t max_len);
uint32_t config_bt_profile_v35_ring_meta(uint8_t *data, uint32_t max_len);
uint32_t config_bt_profile_v35_ring_event(uint8_t logical_index, uint8_t *data, uint32_t max_len);
uint32_t config_bt_profile_v35_find_diag(uint8_t *data, uint32_t max_len);
uint32_t config_bt_profile_v35_find_extra_diag(uint8_t *data, uint32_t max_len);
...
/* Create a MAC-keyed profile when a controller first connects. */
int32_t config_bt_profile_ensure(uint8_t dev_id, uint8_t out_idx);
int32_t config_bt_profile_update_identity(uint8_t dev_id);
uint32_t config_bt_profile_identity_diag(uint8_t profile_id, uint8_t *data, uint32_t max_len);
uint32_t config_bt_profile_persistence_diag(uint8_t profile_id, uint8_t *data, uint32_t max_len);

/* Read/write a persistent profile mapping by profile index. */
...
uint32_t config_bt_profile_export_one(uint8_t profile_id, uint8_t *data, uint32_t max_len);
