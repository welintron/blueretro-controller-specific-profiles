...
void bt_smp_hdlr(struct bt_dev *device, struct bt_hci_pkt *bt_hci_acl_pkt, uint32_t len) {
    switch (bt_hci_acl_pkt->smp_hdr.code) {
        ...
        {
            struct bt_smp_ident_addr_info *ident_addr =
                (struct bt_smp_ident_addr_info *)bt_hci_acl_pkt->smp_data;

            printf("# BT_SMP_CMD_IDENT_ADDR_INFO\n");

            bt_addr_le_copy(&device->le_identity_addr, &ident_addr->addr);
            device->le_identity_valid = 1;
