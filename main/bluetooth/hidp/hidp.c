...
static const struct bt_name_type bt_name_type[] = {
    ...
    {"8BitDo P30 Modkit", BT_HID_GENERIC, BT_SUBTYPE_DEFAULT, 0},
    {"8BitDo P30 classic Modkit", BT_HID_GENERIC, BT_SUBTYPE_DEFAULT, 0},
    {"8BitDo P30 Classic Modkit", BT_HID_GENERIC, BT_SUBTYPE_DEFAULT, 0},
    {"8BitDo PS1 Modkit", BT_HID_GENERIC, BT_SUBTYPE_DEFAULT, 0},
...
void bt_hid_set_type_flags_from_name(struct bt_dev *device, const char* name) {
    ...
    if (device && device->ids.id >= 0 && device->ids.id < BT_MAX_DEV) {
        bt_data = &bt_adapter.data[device->ids.id];
        memset(bt_data->base.raw_name, 0, sizeof(bt_data->base.raw_name));
        if (name) {
            strncpy(bt_data->base.raw_name,
                    name,
                    sizeof(bt_data->base.raw_name) - 1);
        }
    }
    ...
    if (device) {
        config_bt_profile_update_identity(device->ids.id);
    }
}
