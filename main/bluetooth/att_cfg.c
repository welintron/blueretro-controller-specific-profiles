...
#define CFG_CMD_GET_BT_PROFILES 0x08
#define CFG_CMD_GET_BT_PROFILE 0x09
#define CFG_CMD_GET_BT_PROFILE_CFG 0x0A
#define CFG_CMD_SET_BT_PROFILE_CFG 0x0B
#define CFG_CMD_COMMIT_BT_PROFILE_CFG 0x0C
#define CFG_CMD_GET_BT_PROFILE_IDENTITY 0x0D
#define CFG_CMD_GET_BT_PROFILE_PERSISTENCE 0x0E
#define CFG_CMD_GET_BT_PROFILE_APPLY 0x0F
...
static int16_t bt_profile_in_cfg_id = -1;
static uint32_t mc_offset = 0;
static uint8_t cfg_cmd = 0;
static uint8_t bt_profile_query_id = 0;
...
static void bt_att_cmd_in_cfg_rd_rsp(uint16_t handle, uint16_t offset) {
    ...
    if (bt_profile_in_cfg_id >= 0) {
        cfg_len = sizeof(struct in_cfg);
        if (sum_offset > cfg_len || offset > ATT_MAX_LEN) {
            len = 0;
        }
        else {
            len = cfg_len - sum_offset;
            if (len > (mtu - 1)) len = mtu - 1;
            if ((offset + len) > ATT_MAX_LEN) len = ATT_MAX_LEN - offset;
            len = config_bt_profile_read((uint8_t)bt_profile_in_cfg_id,
                                          bt_hci_pkt_tmp.att_data,
                                          sum_offset, len);
        }
    }
    else {
        cfg_len = (sizeof(config.in_cfg[0]) -
                   (ADAPTER_MAPPING_MAX * sizeof(config.in_cfg[0].map_cfg[0]) -
                    config.in_cfg[in_cfg_id].map_size *
                    sizeof(config.in_cfg[0].map_cfg[0])));
        if (sum_offset > cfg_len || offset > ATT_MAX_LEN) {
            len = 0;
        }
        else {
            ...
            if (len > (mtu - 1)) len = mtu - 1;
            if ((offset + len) > ATT_MAX_LEN) len = ATT_MAX_LEN - offset;
            memcpy(bt_hci_pkt_tmp.att_data,
                   (void *)&config.in_cfg[in_cfg_id] + sum_offset, len);
        }
    }
...
static void bt_att_cfg_cmd_bt_profiles_rsp(uint16_t handle)
{
    /*
     * Return only the profile count. A full profile list can exceed the
     * negotiated ATT MTU. Individual profiles are fetched with 0x09.
     */
    bt_hci_pkt_tmp.att_data[0] = (uint8_t)config_bt_profile_get_count();
    bt_att_cmd(handle, BT_ATT_OP_READ_RSP, 1);
}

static void bt_att_cfg_cmd_bt_profile_rsp(uint16_t handle)
{
    /*
     * Profile records are 45 bytes in V12:
     * id + valid + system + source + out + addr_type + MAC + name_len + name.
     */
    uint8_t tmp[45];
    uint32_t len = config_bt_profile_export_one(
        bt_profile_query_id, tmp, sizeof(tmp));

    if (len == 0 || bt_profile_query_offset >= len) {
        bt_att_cmd(handle, BT_ATT_OP_READ_RSP, 0);
        return;
    }

    len -= bt_profile_query_offset;
    if (len > 20) {
        len = 20;
    }

    memcpy(bt_hci_pkt_tmp.att_data,
           tmp + bt_profile_query_offset, len);
    bt_att_cmd(handle, BT_ATT_OP_READ_RSP, len);
}

static void bt_att_cfg_cmd_bt_profile_identity_rsp(uint16_t handle)
{
    uint8_t tmp[19];
    uint32_t len = config_bt_profile_identity_diag(
        bt_profile_query_id, tmp, sizeof(tmp));
    ...
    memcpy(bt_hci_pkt_tmp.att_data,
           tmp + bt_profile_query_offset, len);
    bt_att_cmd(handle, bt_profile_query_offset ?
               BT_ATT_OP_READ_BLOB_RSP : BT_ATT_OP_READ_RSP, len);
}

static void bt_att_cfg_cmd_bt_profile_persistence_rsp(uint16_t handle)
{
    uint8_t tmp[20];
    uint32_t len = config_bt_profile_persistence_diag(
        bt_profile_query_id, tmp, sizeof(tmp));
...
static void bt_att_cfg_cmd_bt_profile_apply_rsp(uint16_t handle)
{
    uint8_t tmp[20];
    uint32_t len = config_bt_profile_apply_diag(tmp, sizeof(tmp));

    memcpy(bt_hci_pkt_tmp.att_data, tmp, len);
    bt_att_cmd(handle, BT_ATT_OP_READ_RSP, len);
}

static void bt_att_cfg_cmd_bt_profile_addr_diag_rsp(uint16_t handle)
{
    uint8_t tmp[20];
    uint32_t len = config_bt_profile_addr_diag(tmp, sizeof(tmp));
    ...
    memcpy(bt_hci_pkt_tmp.att_data, tmp + bt_profile_query_offset, len);
    bt_att_cmd(handle,
               bt_profile_query_offset ? BT_ATT_OP_READ_BLOB_RSP : BT_ATT_OP_READ_RSP,
               len);
}

static void bt_att_cfg_cmd_bt_profile_hash_diag_rsp(uint16_t handle)
{
    uint8_t tmp[20];
    uint32_t len = config_bt_profile_hash_diag(tmp, sizeof(tmp));
...
static void bt_att_cfg_cmd_bt_profile_nvs_diag_rsp(uint16_t handle)
{
    uint8_t tmp[24];
    uint32_t len = config_bt_profile_nvs_diag(tmp, sizeof(tmp));
...
static void bt_att_cfg_cmd_bt_profile_cfg_rsp(uint16_t handle)
{
    ...
    if (bt_profile_in_cfg_id >= 0) {
        read_len = config_bt_profile_read((uint8_t)bt_profile_in_cfg_id,
                                           tmp, in_cfg_offset, sizeof(tmp));
        if (read_len > 0) {
    ...
    bt_att_cmd(handle, in_cfg_offset ? BT_ATT_OP_READ_BLOB_RSP : BT_ATT_OP_READ_RSP, len);
}
...
static void bt_att_cfg_cmd_rd_hdlr(uint16_t handle) {
    switch (cfg_cmd) {
        ...
        case CFG_CMD_GET_FW_NAME:
            ...
            break;
        case CFG_CMD_GET_BT_PROFILES:
            bt_att_cfg_cmd_bt_profiles_rsp(handle);
            break;
        case CFG_CMD_GET_BT_PROFILE:
            bt_att_cfg_cmd_bt_profile_rsp(handle);
            break;
        case CFG_CMD_GET_BT_PROFILE_IDENTITY:
            bt_att_cfg_cmd_bt_profile_identity_rsp(handle);
            break;
        case CFG_CMD_GET_BT_PROFILE_PERSISTENCE:
            bt_att_cfg_cmd_bt_profile_persistence_rsp(handle);
            break;
        case CFG_CMD_GET_BT_PROFILE_APPLY:
            bt_att_cfg_cmd_bt_profile_apply_rsp(handle);
            break;
        case CFG_CMD_GET_BT_PROFILE_ADDR_DIAG:
            bt_att_cfg_cmd_bt_profile_addr_diag_rsp(handle);
            break;
        case CFG_CMD_GET_BT_PROFILE_HASH_DIAG:
            bt_att_cfg_cmd_bt_profile_hash_diag_rsp(handle);
            break;
...
static void bt_att_cfg_cmd_wr_hdlr(struct bt_dev *device, uint8_t *data, uint32_t len) {
    ...
    else if (cfg_cmd == CFG_CMD_GET_BT_PROFILE_CFG ||
             cfg_cmd == CFG_CMD_SET_BT_PROFILE_CFG ||
             cfg_cmd == CFG_CMD_COMMIT_BT_PROFILE_CFG) {
        bt_profile_in_cfg_id = (len > 1) ? (int16_t)data[1] : -1;
        in_cfg_offset = (len > 3) ?
            (uint16_t)(data[2] | ((uint16_t)data[3] << 8)) : 0;
    }
    ...
    switch (cfg_cmd) {
        case CFG_CMD_COMMIT_BT_PROFILE_CFG:
            if (bt_profile_in_cfg_id >= 0 &&
                config_bt_profile_commit((uint8_t)bt_profile_in_cfg_id) < 0) {
                printf("# %s: profile commit failed: %d\\n", __FUNCTION__,
                       bt_profile_in_cfg_id);
            }
    ...
    switch (bt_hci_acl_pkt->att_hdr.code) {
        ...
        case BT_ATT_OP_WRITE_REQ:
        {
            ...
            switch (wr_req->handle) {
                ...
                case BR_IN_CFG_DATA_CHRC_HDL:
                    if (bt_profile_in_cfg_id >= 0) {
                        config_bt_profile_write((uint8_t)bt_profile_in_cfg_id,
                                                wr_req->value,
                                                in_cfg_offset,
                                                data_len);
                    }
                    else {
                        memcpy((void *)&config.in_cfg[in_cfg_id] + in_cfg_offset,
                               wr_req->value, data_len);
                        config_update(config_get_src());
                    }
                    bt_att_cmd_wr_rsp(device->acl_handle);
                    break;
